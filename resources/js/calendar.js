...
const calendar = new Calendar(calendarEl, {
    ...
    // 予定がない部分をクリック
    selectable: true,  // trueしないと選択できない
    ...
    // 予定時刻のサイズ変更
    editable: true,  // trueのしないと変更できない
    eventResize: function (event, delta) {
        alert('eventResize: のイベントです');
    },
});
calendar.render();
...
const formAllDay = modalForm.querySelector('input[name="all-day"]');
...
const formGuestCount = modalForm.querySelector('input[name="guest_count"]');
...
function editModal(event) {
    ...
    formGuestCount.value = event.extendedProps.guest_count;
    // ボタンの表示/非表示
...
addButton.addEventListener('click', function () {
    ...
    const data = {
        ...
        body: formBody.value,
        guest_count: formGuestCount.value,
    ...
    axios.post('/calendar/action', data)
        .then((responce) => {
            // 予定をカレンダーに追加
            calendar.addEvent(responce.data);
